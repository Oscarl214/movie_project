...
const [searchQuery,setSearchQuery]=useState('')
  ...
  const {data:movies,loading,error, refetch: loadMovies, reset}=useFetch(()=>getMovies({query:searchQuery}),false)
  ...
  return (
   ...
   <FlatList data={movies?.results} renderItem={({item})=> <MovieCard {...item} />}
    ...
    className='px-5' numColumns={3}
     ...
     ListEmptyComponent={
      ...
      !loading && !error ? (
        <View className='mt-10 px-5'><Text className='text-center text-gray-500'>
